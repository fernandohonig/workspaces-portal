...
exports.handler = (event, context, callback) => {
    ...
    if (action == "list") {
        ...
        console.log(
            "Trying to find desktop owned by: " +
        ...
        workspaces.describeWorkspaces(describeWorkspacesParams, function (
            err,
            data
        ) {
            if (err) {
                ...
                for (var i = 0; i < data.Workspaces.length; i++) {
                    ...
                    (function (describeTagsParams) { // Limit scope to avoid describeTagsParams getting overwritten too fast.
                        workspaces.describeTags(describeTagsParams, function (err, data) {
                            if (err) {
                                console.log(err, err.sZtack);
                            } else {
                                for (var i = 0; i < data.TagList.length; i++) {
                                    ...
                                    if (
                                        data.TagList[i].Key == "SelfServiceManaged" &&
                                        data.TagList[i].Value ==
                                        event.requestContext.authorizer.claims.email
                                    ) {
                                        console.log(
                                            "Desktop for '" +
                                        ...
                                        var describeDetailsParams = {
                                            WorkspaceIds: [describeTagsParams.ResourceId]
                                        };

                                        workspaces.describeWorkspaces(
                                            describeDetailsParams,
                                            function (err, data) {
                                                if (err) {
        ...
        lambda.invoke(detailsParams, function (err, data) {
            if (err) {
                ...
                for (var i = 0; i < JSON.parse(data.Payload).length; i++) {
                    console.log(
                        "Username #" + i + ": " + JSON.parse(data.Payload)[i].Username.S
                    );
                }
                        ...
                        if (err) {
                            ...
                            for (var i = 0; i < data.TagList.length; i++) {
                                if (
                                    ...
                                    workspaces.rebuildWorkspaces(rebuildParams, function (
                                        err,
                                        data
                                    ) {
                                        if (err) {
                        ...
                        if (err) {
                            ...
                            for (var i = 0; i < data.TagList.length; i++) {
                                if (
                                    ...
                                    workspaces.rebootWorkspaces(rebootParams, function (
                                        err,
                                        data
                                    ) {
                                        if (err) {
                        ...
                        if (err) {
                            ...
                            for (var i = 0; i < data.TagList.length; i++) {
                                if (
                                    ...
                                    workspaces.terminateWorkspaces(deletionParams, function (
                                        err,
                                        data
                                    ) {
                                        if (err) {
            ...
            workspaces.describeWorkspaceBundles(parameters, function (err, data) {
                if (err) {
                    ...
                    for (var i = 0; i < data["Bundles"].length; i++) {
                        console.log(
                            data["Bundles"][i].BundleId + ":" + data["Bundles"][i].Name
                        );
                        bundleList.push(
                            data["Bundles"][i].BundleId + ":" + data["Bundles"][i].Name
                        );
